...
def parse_arguments():
    ...
    parser.add_argument(
        "--fasta_path", type=str, help="Path to fasta file of enzyme sequences"
    )
    parser.add_argument(
        "--save_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to directory containing pre-downloaded model weights",
    )
...
def setup_model_weights(weights_dir=None):
    ...
    weight_files = ["esm1v_t33_650M_UR90S_1.pt", "ESM1v-RLATtr.pt"]
...
def write_attention_weights(
    accs, seqs, attention_weights, attention_dir, attention_mode="average"
):
    ...
    for i, (acc, seq) in enumerate(zip(accs, seqs)):
        ...
        weights = pd.DataFrame(
            weights.transpose(), index=list(seq), columns=["weights"]
        )
...
class EpHodModel:
    ...
    def get_ESM1v_embeddings(self, accs, seqs):
        ...
        emb = emb.transpose(
            2, 1
        )  # From (batch, seqlen, features) to (batch, features, seqlen)
    ...
    def load_RLAT_model(self):
        ...
        model = nn_models.ResidualLightAttention(
            dim=1280, kernel_size=7, dropout=0.0, res_blocks=4, activation="elu"
        )
        model = model.to(self.device)
        url = "https://zenodo.org/records/14252615/files/ESM1v-RLATtr.pt?download=1"
        model_dict = torch.hub.load_state_dict_from_url(
            url, progress=False, map_location=self.device
        )
        model_dict = {
            key[len("module.") :]: value for key, value in model_dict.items()
        }  # Remove DataParallel suffix
    ...
    def predict(self, accs, seqs):
        ...
        masks = [
            [1] * len(seqs[i]) + [0] * (maxlen - len(seqs[i])) for i in range(len(seqs))
        ]
        ...
        emb_pool = (emb_pool - self.svr_stats[:, 0]) / (
            self.svr_stats[:, 1] + 1e-8
        )  # Normalize with means/std.dev
        svr_pred = self.svr_model.predict(
            emb_pool
        )  # Note that batch size > 1 affects this pooling
...
def main():
    ...
    headers, sequences, accessions = [
        np.array(item) for item in (headers, sequences, accessions)
    ]
    assert (
        len(accessions) == len(headers) == len(sequences)
    ), "Fasta file has unequal headers and sequences"
        ...
        for batch_step in batches:
            ...
            all_ypred = np.vstack(
                (
                    all_ypred,
                    np.array(
                        [out["rlat_pred"], out["svr_pred"], out["ensemble_pred"]]
                    ).transpose(),
                )
            )
    ...
    all_ypred = pd.DataFrame(
        all_ypred, index=accessions, columns=["RLATtr", "SVR", "Ensemble"]
    )
